...
const protectedPaths = ["/dashboard", "/playlists/new"];
...
export async function proxy(request: NextRequest) {
  ...
  if (protectedPaths.includes(request.nextUrl.pathname) && !user) { const login = request.nextUrl.clone(); login.pathname = "/auth"; login.searchParams.set("next", request.nextUrl.pathname); return NextResponse.redirect(login); }
  return response;
}

export const config = { matcher: ["/dashboard", "/playlists/new"] };
